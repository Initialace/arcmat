...
export const useGetProducts = (userId) => {
    return useQuery({
        queryKey: PRODUCT_KEYS.list({ userId }),
        queryFn: () => productService.getAllProducts({ user_id: userId }),
        // If userId is missing, we might want to skip or fetch all (depending on logic)
        // For now enabled always, service handles if params are empty
    });
};
