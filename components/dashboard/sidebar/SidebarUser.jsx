...
const SidebarUser = memo(({ isCollapsed, mounted }) => {
    ...
    // Debug logging
    useEffect(() => {
        console.log('SidebarUser - User changed:', user);
        console.log('SidebarUser - User role:', user?.role);
        console.log('SidebarUser - User name:', user?.name || user?.fullName);
    }, [user]);

    const getUserRoleLabel = () => {
        if (!user?.role) {
            console.log('SidebarUser - No role detected, returning Guest');
            return 'Guest';
        }
        switch (user.role) {
            case 'vendor':
